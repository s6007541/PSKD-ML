...
class Multi_ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, num_resnet_blocks=4):
        super(Multi_ResNet, self).__init__()
        self.num_resnet_blocks = num_resnet_blocks
        if self.num_resnet_blocks == 1:
            self.dim_final_fea = 64
        elif self.num_resnet_blocks == 2:
            self.dim_final_fea = 128
        elif self.num_resnet_blocks == 3:
            self.dim_final_fea = 256
        elif self.num_resnet_blocks == 4:
            self.dim_final_fea = 512
        else:
        ...
        self.bottleneck1_1 = branchBottleNeck(64 * block.expansion, self.dim_final_fea * block.expansion, kernel_size=8)
        self.avgpool1 = nn.AdaptiveAvgPool2d((1,1))
        self.middle_fc1 = nn.Linear(self.dim_final_fea * block.expansion, num_classes)
        ...
        self.bottleneck2_1 = branchBottleNeck(128 * block.expansion, self.dim_final_fea * block.expansion, kernel_size=4)
        self.avgpool2 = nn.AdaptiveAvgPool2d((1,1))
        self.middle_fc2 = nn.Linear(self.dim_final_fea * block.expansion, num_classes)
        ...
        self.bottleneck3_1 = branchBottleNeck(256 * block.expansion, self.dim_final_fea * block.expansion, kernel_size=2)
        self.avgpool3 = nn.AdaptiveAvgPool2d((1,1))
        self.middle_fc3 = nn.Linear(self.dim_final_fea * block.expansion, num_classes)

        self.avgpool = nn.AdaptiveAvgPool2d((1,1))
        
        self.fc = nn.Linear(self.dim_final_fea * block.expansion, num_classes)
...
def multi_resnet50_kd(num_classes=100, num_resnet_blocks=4):
    return Multi_ResNet(BottleneckBlock, [3,4,6,3], num_classes=num_classes)

def multi_resnet18_kd(num_classes=100, num_resnet_blocks=4):
    return Multi_ResNet(BasicBlock, [2,2,2,2], num_classes=num_classes, num_resnet_blocks = num_resnet_blocks)


if __name__=="__main__":
    import torchsummary
    
    model = multi_resnet50_kd(num_classes=100, num_resnet_blocks=4)
    model = model.cuda()
    report = torchsummary.summary(model, input_size=(3,256,256))
